...
def do_the_magic_stuff(arguments):
    ...
    if "--mode" in sys.argv:
        ...
        if args[args.index("--mode") + 1] == "pre" \
            ...
            if not overwriting:
                if os.path.exists(args[args.index("--outpath") + 1]) and os.path.isdir(
                        args[args.index("--outpath") + 1]):
                    if os.path.exists(args[args.index("--outpath") + 1] + "first.vcf") and not overwriting:
                        sys.exit(args[args.index(
                            "--outpath") + 1] + "first.vcf is already existing and overwriting is deactivated.")
                    if os.path.exists(args[args.index("--outpath") + 1] + "second.vcf") and not overwriting:
                        sys.exit(args[args.index(
                            "--outpath") + 1] + "second.vcf is already existing and overwriting is deactivated.")
            try:
            ...
            if not overwriting:
                if os.path.exists(args[args.index("--outpath") + 1]) and os.path.isdir(
                        args[args.index("--outpath") + 1]):
                    if os.path.exists(
                            args[args.index("--outpath") + 1] + "all_transcripts_data.fa") and not overwriting:
                        sys.exit(args[args.index(
                            "--outpath") + 1] + "all_transcripts_data.fa is already existing and "
                                                "overwriting is deactivated.")
                    if os.path.exists(args[args.index("--outpath") + 1] + "All_VCF.vcf") and not overwriting:
                        sys.exit(args[args.index(
                            "--outpath") + 1] + "All_VCF.vcf is already existing and overwriting is deactivated.")
            try:
                os.makedirs(args[args.index("--outpath") + 1], exist_ok=True)
            except FileExistsError:
                pass
                    ...
                    for picture_format in picture_formats.split(','):
                        if picture_format.lower() in probably_supported_formats:
                            try:
                                ...
                                sys.exit("File is existing and overwriting is not active:" + outputfolder +
                                         orig_outputname.split(".")[0] + "." + picture_format.lower())
                            except FileNotFoundError:
                                continue
