...
def find_all_cindels(vcf_file_link: str, mod_or_not: bool, output_folder: str, max_bp_range: int):
    ...
    for transcripts in transcript_indels_dict.keys():
        ...
        if len(t_entrys) > 1:
            ...
            for entry in t_entrys:
                shift += entry[1]
                # if this reaches 0 (or mod 3 == 0), the frameshift is (maybe, no stop detection here) compensated
                sub_indel_list.append(entry)
                # all entrys needed for one additional compensated fs (could be more than one)
                if mod_or_not:
                    if shift % 3 == 0:
                        # compensation
                        for sub_indels in sub_indel_list:
                            # all fs entrys
                        ...
                        sub_indel_list = []
                    # search for next compensating fs
                if not mod_or_not:
                    if shift == 0:
                        # compensation
                        for sub_indels in sub_indel_list:
    ...
    table_output = []
    unique_table_output = []
    max_bp_between_compensation_fs: int
    for max_bp_between_compensation_fs in range(1, max_bp_range + 1):
        isoform_dict = {}
        unique_dict = {}
        for transcripts in possible_neutralizing_indel_transcripts.keys():
            ...
            range_check = 0
            too_long = False
            # True, if length between indels is too large
            for i, entrys in enumerate(possible_neutralizing_indel_transcripts[transcripts][1:]):
                ...
                if range_check >= max_bp_between_compensation_fs:
                    too_long = True
                    break
            if too_long:
                continue
        ...
        number_of_zeros = 1
        table_output.append(str(max_bp_between_compensation_fs))
        ...
        list_of_keys = sorted(list_of_keys)
        number_of_zeros = 1
        unique_table_output.append(str(max_bp_between_compensation_fs))
        for i in list_of_keys:
    ...
    maxlength = len(formating_list[len(formating_list) - 1].split('\t'))
    # length of last entry
    new_output = []
    for line in formating_list:
        # line is a string for one line
        new_output.append(line)
        diff = abs(maxlength - len(line.split("\t")))
        # if there are not enough zeros, there should be a difference
        if diff > 0:
    ...
    description = "#transcripts with X indels (cumulative) - {0} - to max distance:{1}\n".format(
        str(mode_name), str(max_bp_range))
    description += "#max_bp_between_compensation_fs" + "".join(key_output) + "\n"

    table_output_file = open(output_folder + table_outputname, 'w')
    ...
    description2 = "#transcripts with X indels (cumultative) - unqiue dataset - {0} - to max distance:{1}\n".format(
        str(mode_name), str(max_bp_range))
    description2 += "#max_bp_between_compensation_fs" + "".join(key_output) + "\n"

    table_output_file = open(output_folder + unique_table_outputname, 'w')
...
def do_magic_plotting(data: list, outputfolder: str, orig_outputname: str, formats: str, max_x_axis_bpr: int):
    ...
    for dataline in data[0:60]:
        ...
        for i in range(1, max_coloumns_so_bars):
            all_bars[i].append(int(dataline[i]))
    # involved transcripts with cindel_events
    # create all bars
    legend = []
    for i in range(1, max_coloumns_so_bars):
...
def find_all_cindels_v2(navip_vcf_file_link: str, output_folder: str, mod_or_not: bool, formats: str,
                        max_x_axis_bpr: int):
            ...
            for entry in t_entrys:
                ...
                if mod_or_not:
                    if shift % 3 == 0:  # compensation
                        ...
                        # new_cindel : <class 'tuple'>: ('AT1G02020.1', 2, [(16, 2, 'Chr1', 354830), (43, 1, 'Chr1',
                        # 354811)], 27)
                        ...
                        if max_dist in possible_neutralizing_indel_transcripts_experimental:
                            if len(sub_indel_list) in possible_neutralizing_indel_transcripts_experimental[max_dist]:
                                ...
                                possible_neutralizing_indel_transcripts_experimental[max_dist][len(sub_indel_list)] = [
                                    new_cindel]
                                add_this_without_bug = str(new_cindel_unique[0]) + "\t" + str(
                                    new_cindel_unique[1]) + "\t" + str(new_cindel_unique[2]) + "\t" + str(
                                    new_cindel_unique[3])
    ...
    for max_base_pairs in sorted(list(possible_neutralizing_indel_transcripts_experimental.keys())):
        ...
        for cindelsLength in sorted(list(possible_neutralizing_indel_transcripts_experimental[max_base_pairs])):
            ...
            normal_output.append(
                "\t" + str(len(possible_neutralizing_indel_transcripts_experimental[max_base_pairs][cindelsLength])))
    ...
    for mbp_clength_temp_entry in involved_tid_list:
        for tid in mbp_clength_temp_entry[2]:
            involved_tid_list_unique_set.add(
                str(mbp_clength_temp_entry[0]) + ' ' + str(mbp_clength_temp_entry[1]) + ' ' + str(tid.split(".")[0]))
    involved_tid_list_unique = list(involved_tid_list_unique_set)
    involved_tid_list_unique = sorted(involved_tid_list_unique, key=lambda x: (
        int(x.split(" ")[0]), int(x.split(" ")[1])))  # [000]:'1 2 AT1G22060'

    involved_tid_list_output = []
    old_header = "> " + str(involved_tid_list_unique[0].split(" ")[0]) + " " + str(
        involved_tid_list_unique[0].split(" ")[1] + "\n")
    ...
    for mbp_clength_temp_entry in involved_tid_list_unique:
        ...
        new_header = "> " + str(mbp_clength_temp_entry.split(" ")[0]) + " " + str(
            mbp_clength_temp_entry.split(" ")[1] + "\n")
        ...
        for quantity_of_involved_indels in possible_neutralizing_indel_transcripts_experimental[bpr]:
            ...
            for cindel_event in possible_neutralizing_indel_transcripts_experimental[bpr][quantity_of_involved_indels]:
                ...
                for data_tuple in cindel_event[2]:
                    # <class 'tuple'>: ('AT1G06220.1', 2, [(350, 2, 'Chr1', 1900873), (353, 1, 'Chr1', 1900874)], 3)
                    cindel_details += str(data_tuple[2]) + "," + str(data_tuple[3]) + "," + str(
                        data_tuple[0]) + "," + str(data_tuple[1]) + ";"
                # cindel_details += "|"
            ...
            for cindel_details_list in detailed_output_stuff_dict[(bpr, quantity)][tid]:
                ...
                for cindel_event in cindel_events_list:
                    ...
                    involved_tid_list_detailed_output.append(
                        cindel_event[0:len(cindel_event) - 1])  # -1 because of ignoring the last ";"
                    if i != 0:
                        involved_tid_list_detailed_output.append("|")
            involved_tid_list_detailed_output.append("\n")
    description = "#Header: > <bp> <quantity of involved InDels for one compensating InDel (cInDel) event>  \n"
    description += "#Data line: <tid|cInDel-event 1| cInDel-event2| ....> \n"
    description += "#cInDel-event:<Chr>,<Pos>,<CDS-Pos>,<frameshift-value>;<Chr>,<Pos>,<CDS-Pos>,"
    description += " <frameshift-value> [...]\n"
    ...
    description = ""
    description += "#Data line: <tid>,<quantity cindel-events>|<max_bpr>,"
    description += "<InDel_quantity>,cInDel-event 1|<bpr><InDel_quantity>,cInDel-event2| ....> \n"
    description += "#cInDel-event:<Chr>,<Pos>,<CDS-Pos>,<frameshift-value>;<Chr>,<Pos>,"
    description += "<CDS-Pos>,<frameshift-value> [...]\n"
    ...
    print("### Overview ###")
    print("Number of transcripts (+isoforms) with at least one compensation InDel (cInDel) event: " + str(
        len(sort_this_tids)))
    ...
    print("Number of transcripts (unique) with more then one compensation InDel (cInDel) event: " + str(
        len(counti_unique_list)))
