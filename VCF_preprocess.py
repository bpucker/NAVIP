...
def preprocessing_original_vcf_file(ori_vcf: str, new1_vcf: str, new2_vcf: str, outpath: str):
    ...
    def format_multiallel_variant(splitline: list) -> str:
        """
        ...
        if reflen == 1 and altlen == 1:
            ...
            # can maybe remove this case, if everything worked fine
            nref = splitline[3]
            nalt = splitline[4]
            skipped = 0
            # remove all starting bases, if they are identical
    ...
    lines1 = []
    lines2 = []
    format_multiallel_variant_warning = True
    while line:
        if line.startswith("#"):
            infos.append(line)
            line = ori_vcf_file.readline()
        ...
        if len(splitline) == 1:
            ...
            erg = format_multiallel_variant(new_entry)
            if not erg.startswith("#"):
                lines1.append(erg)
            elif format_multiallel_variant_warning:
                format_multiallel_variant_warning = False
            ...
            if len(splitline[3]) > 1 and len(splitline[4]) > 1:
                ...
                new_entry.append("\t".join(splitline[5:]))
                line = format_multiallel_variant(new_entry)
            lines1.append(line)
            lines2.append(line)
        line = ori_vcf_file.readline()
    ori_vcf_file.close()
